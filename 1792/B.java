...
public class B {
    ...
    public static void solve() {
       ...
       long ans = 0;
       long min = Math.min(arr[1], arr[2]);
       long max = Math.max(arr[1], arr[2]);
       // out.println(" max " + max + " " + min);
       if(Math.max(arr[1], arr[2]) > Math.min(arr[1], arr[2])){
       	ans = arr[0] + Math.min(arr[1], arr[2])*2 + Math.min(arr[0] + 1, (max - min) + arr[3]);
       }else{
       	ans = arr[0] + Math.min(arr[1], arr[2])*2 + Math.min(arr[0] + 1, arr[3]);
       }
       out.println(ans);
    }
