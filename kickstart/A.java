...
public class A {
    ...
    public static void solve(int t) {
         ...
         for(int i = 0; i < n; i++){
            ...
            d[i][0] = sc.nextInt();
            idx[i] = sc.nextInt();
            color[i] = new Pair(c, idx[i]);
            d[i][1] = idx[i];
          }

          Arrays.sort(color, (a, b) -> a.s.equals(b.s) ? a.indx - b.indx : a.s.compareTo(b.s));
          Arrays.sort(d, (a,b) -> a[0] == b[0] ? a[1] - b[1]: a[0] - b[0]);

          int cnt = 0;
          for(int i = 0; i < n; i++){
            if(color[i].indx == d[i][1]) cnt++;
          }
