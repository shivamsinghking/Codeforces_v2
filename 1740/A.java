...
public class A {
    ...
    public static void solve() {
        int n = sc.nextInt();
        int M  = sc.nextInt();
        int[] L = sc.readArrayInt(n);
        int[] R = sc.readArrayInt(n);
        ...
        for(int i = 0; i < n; i++){
        	ll.add(new int[]{L[i], 1});
        	ll.add(new int[]{R[i], -1});
        }
        ...
        int prev = Integer.MAX_VALUE, cnt = 0;
        for(int i = 0; i < ll.size(); i++){
        	...
        	// System.out.println(" v " + v[0] + " " + cnt + " " + prev);
        	if(cnt == M){
        		if(prev != Integer.MAX_VALUE){
        			if(prev <= v[0] - 1){
        				ans.add(new int[]{prev, v[0]-1});
        			}
        			prev = Integer.MAX_VALUE;
        		}
        	}else if(cnt < M){
        		prev = Math.min(prev, v[0]);
        	}
        }
        
        if(prev <= MM){
        	ans.add(new int[]{prev, MM});
        }
