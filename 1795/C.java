...
class Solution {
    ...
    void solve(){
       ...
       for(int i = 0; i < n; i++){
       	prefixsum[i + 1] = prefixsum[i] + brr[i];
       }
       
       long[] store = new long[n + 2];
       long[] cnt = new long[n + 2];
       
       for(int i = 0; i < n; i++){
       	 int v = arr[i];
       	 int idx = lower_bound(prefixsum, v, i);
       	 long total = prefixsum[idx + 1] - prefixsum[i];
       	 long left = v - total;
       	 if(left < 0) continue;
       	 store[idx + 1] += left;
       	 cnt[idx + 1]--;
       ...
       for(int i = 0; i < n; i++){
       	 cc += cnt[i];
       	 long t = (i + 1 + cc)*(long)brr[i] + store[i];
       	 ans[i] = t;
       }
     ...
     public static int lower_bound(long[] ar, long k, int start) {
        int s = start, e = ar.length - 1;
        while (s != e) {
            int mid = s + e >> 1;
            if (ar[mid + 1] - ar[start] <= k) {
                s = mid + 1;
            } else {
                e = mid;
            }
