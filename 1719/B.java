...
public class B {
    ...
    public static void solve() {
        ...
        //  int cnt = 0;
         ...
         if(q.size() > q1.size() + q2.size()){
          out.println("NO");
          return;
         }

         List<int[]> ll = new ArrayList<>();
         while(q1.size() > 0 && q.size() > 0){
             ll.add(new int[]{q1.poll(), q.poll()});
         }

         while(q.size() > 0 && q2.size() > 0){
            ll.add(new int[]{q.poll(), q2.poll()});
         }

         while(q1.size() > 0 && q2.size() > 0){
           ll.add(new int[]{q1.poll(), q2.poll()});
         }

         while(q1.size() > 0){
          ll.add(new int[]{q1.poll(), q1.poll()});
         }

         while(q2.size() > 0){
          ll.add(new int[]{q2.poll(), q2.poll()});
         }
