...
class Solution {
    ...
    void solve(){
       int n = sc.nextInt();
       char[] arr = sc.nextLine().trim().toLowerCase().toCharArray();
       ...
       if(set.contains('m') == false || !set.contains('e') ||  !set.contains('o') || set.contains('w') == false){
       	out.println("NO");
       	return;
       }
       ...
       for(int i = 0; i < n; i++){
       	if(arr[i] == 'm'){
       		...
       		if(prev == 'e' || prev == 'o') prev = 'o';
       		else flag = false;
       	}else if(arr[i] == 'w'){
       		if(prev == 'o' || prev == 'w') prev = 'w';
       		else flag = false;
       	}else flag = false;
       }
