...
public class B {
    ...
    public static void solve() {
        ...
        for(int i = 1; i < n; i++) map.put(arr[i], map.getOrDefault(arr[i], 0) + 1);
        
        while(map.higherKey(arr[0]) != null){
        	int k = map.higherKey(arr[0]);
        	
        	map.put(k, map.get(k) - 1);
        	if(map.get(k) == 0) map.remove(k);
        	
        	int nxtValue = (int)Math.ceil((k + arr[0])/2.0);
        	int diff = nxtValue - arr[0];
        	map.put(k - diff, map.getOrDefault(k - diff, 0) + 1);
        	arr[0] = nxtValue;
        }
