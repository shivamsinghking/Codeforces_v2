...
public class E {
    ...
    public static void solve() {
        ...
        for(int i = 1; i <= n && partionNumber <= k; i++){
        	int v = x^i;
        	if(v <= n && store[i] == 0 && store[v] == 0){
        		store[i] = partionNumber;
        		store[v] = partionNumber;
        		partionNumber++; 
        	}
        }
        ...
        for(int i = 1; i <= n; i++){
        	int groupNo = Math.max(store[i], 1);
        	ans.get(groupNo).add(i);
        }
