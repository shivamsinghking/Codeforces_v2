...
public class C {
    ...
    static Stack<long[]> helper(long[] arr, int n, int m){
    	 ...
    	 for(int i = 0; i < n; i++){
        	...
        	long cnt = 1L;
        		...
        		if(st1.size() > 0 && st1.peek()[0] == a){
        			st1.peek()[1] += cnt;
        		}else{
        			st1.push(new long[]{a, cnt});
        		}
        }
    ...
    public static void solve() {
        ...
        int k = sc.nextInt();
        long[] brr = sc.readArrayLong(k);
        
        Stack<long[]> st1 = helper(arr, n, m);
        Stack<long[]> st2 = helper(brr, k, m);
        ...
        while(st1.size() > 0){
        	if(Arrays.equals(st1.peek(), st2.peek()) == false){
        		out.println("NO");
        		return;
        	}
        	st1.pop();
